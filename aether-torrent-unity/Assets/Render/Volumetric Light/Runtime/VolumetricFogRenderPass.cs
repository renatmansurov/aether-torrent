...
namespace Render.Volumetric_Light.Runtime
{
	...
	public sealed class VolumetricFogRenderPass : ScriptableRenderPass
	{
		...
		private static readonly int VolumeNoiseTextureId = Shader.PropertyToID("_VolumeNoiseTexture");
		private static readonly int NoiseScaleId = Shader.PropertyToID("_NoiseScale");
		private static readonly int NoiseSpeedId = Shader.PropertyToID("_NoiseSpeed");
		private static readonly int NoiseHeightInfluenceId = Shader.PropertyToID("_NoiseHeightInfluence");
		private static readonly int NoiseBlendFactorId = Shader.PropertyToID("_NoiseBlendFactor");
		...
		private static void UpdateVolumetricFogMaterialParameters(Material volumetricFogMaterial, int mainLightIndex, int additionalLightsCount, NativeArray<VisibleLight> visibleLights)
		{
			...
			volumetricFogMaterial.SetVector(NoiseScaleId, fogVolume.noiseScale.value);
			volumetricFogMaterial.SetVector(NoiseSpeedId, fogVolume.noiseSpeed.value);
			volumetricFogMaterial.SetFloat(NoiseHeightInfluenceId, fogVolume.noiseHeightInfluence.value);
			volumetricFogMaterial.SetFloat(NoiseBlendFactorId, fogVolume.noiseBlendFactor.value);
			volumetricFogMaterial.SetTexture(VolumeNoiseTextureId, fogVolume.volumeNoiseTexture.value);
		}
